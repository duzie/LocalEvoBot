...
async function getOrCreateMqttUid() {
    ...
    mqttUidPromise = (async () => {
        ...
        const { value } = await pluginSdk.readJsonFileWithFallback(configFilePath, {});
        ...
        await pluginSdk.writeJsonFileAtomically(configFilePath, cfg);
        mqttUid = next;
        return mqttUid;
    })()
        .catch((err) => {
...
const mqttChannelPlugin = {
    ...
    outbound: {
        ...
        sendText: async ({ to, text, accountId }) => {
            const client = mqttClientsByAccountId.get(String(accountId ?? pluginSdk.DEFAULT_ACCOUNT_ID));
            if (!client) {
