=== 文档处理操作规范 ===
1) 大文件处理：使用 read_document_part 分块读取大文件，避免内存溢出；合理设置 max_chars 参数。代码文件不使用，优先使用 read_files_to_analysis_index + query_analysis_index。
...
6) 项目级/多文件代码分析任务：不得将 read_document_part 作为主通道，必须优先使用 deep_analysis_skill 的索引链路。
7) 如果是写入代码，写入完需要测试。
