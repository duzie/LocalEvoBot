...
def release_request_connection():
    """释放当前请求的连接"""
...
@contextmanager
def request_connection_context(reuse_connection: bool = True):
    """请求级别的连接上下文管理器"""
    if reuse_connection:
...
@tool
def enhanced_manage_sqlserver_connections(action: str = "stats", 
                                server: str = None, 
                                database: str = None, 
                                username: str = None, 
    ...
    Args:
        action: 操作类型 - stats（统计）、test（测试连接）、close（关闭连接）、transaction（事务管理）
        server: 服务器地址
        database: 数据库名称
        username: 用户名
        password: 密码
    ...
    try:
        if action == "stats":
            ...
            # 事务管理
            # 这里可以通过上下文管理器实现更复杂的事务操作
            return {
                "success": True,
                "message": "事务上下文管理器已就绪，使用 db_transaction() 上下文管理器",
                "action": action,
                "features": ["db_transaction", "request_connection_context"]
            }
            ...
            return {
                ...
                "action": action,
                "supported_actions": ["stats", "test", "close", "transaction"]
            }
...
@tool
def enhanced_configure_sqlserver(server: str = None, database: str = None, 
