...
def read_file(
    ...
    try:
        ...
        if not os.path.isfile(file_path):
            return _error_payload("not_a_file", f"路径不是文件：{file_path}", tool=tool_name)
        ...
        return _ok_payload(f"成功读取 {lines} 行", **result)
        
    except Exception as e:
        return _error_payload("read_error", f"读取失败：{e}", tool=tool_name, file=file_path)
