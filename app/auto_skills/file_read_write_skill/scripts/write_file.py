...
def write_file(
    ...
    try:
        # 验证路径
        if not file_path:
            return _error_payload("invalid_path", "文件路径不能为空", tool=tool_name)
        ...
        return _ok_payload(f"成功写入 {file_size} 字节", **result)
        
    except Exception as e:
        return _error_payload("write_error", f"写入失败：{e}", tool=tool_name, file=file_path)
...
def append_file(
    ...
    try:
        ...
        return _ok_payload(f"成功追加内容，总大小 {file_size} 字节", size=file_size)
        
    except Exception as e:
        return _error_payload("append_error", f"追加失败：{e}", tool=tool_name, file=file_path)
