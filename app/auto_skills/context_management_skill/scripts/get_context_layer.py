...
@tool
def get_context_layer(file_path: str = None, layer: str = "summary", query: str = "", max_results: int = 5):
    """
    获取指定层的上下文内容
    Args:
        file_path: 文件路径
        ...
        max_results: 最大返回结果数
    """
    tool_name = "get_context_layer"
    try:
        _emit_event(tool_name, "start", file_path=file_path, layer=layer, query=query)
        
        if not file_path:
            return _error_payload("missing_param", "缺少文件路径参数", tool=tool_name)
        
        # 验证层参数
        valid_layers = ["raw", "summary", "semantic", "metadata"]
        ...
        # 获取上下文层
        result = context_manager.get_context_layer(
            file_path=file_path,
            layer=layer,
            query=query,
