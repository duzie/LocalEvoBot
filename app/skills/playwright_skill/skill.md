...
- playwright_click_and_wait_download: 点击并等待下载完成，保存文件到本地
- playwright_fill: 填充表单元素
- playwright_type: 输入文本到页面元素
- playwright_press: 发送键盘按键（Enter/Escape 等）
- playwright_get_text: 获取元素文本
- playwright_execute_js: 执行JavaScript代码
- playwright_wait_for_selector: 等待元素出现/消失/可见/隐藏
- playwright_wait_for_url: 等待URL包含子串或匹配正则
- playwright_wait_for_response: 等待指定接口响应出现
...
- playwright_switch_page: 切换到指定标签页
...
- playwright_detect_blockers: 检测登录/验证码等阻塞
