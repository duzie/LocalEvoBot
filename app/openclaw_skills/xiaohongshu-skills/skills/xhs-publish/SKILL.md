...
**所有发布操作只能通过本项目的 `python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py` 完成，不得使用任何外部项目的工具：**

- **唯一执行方式**：只运行 `python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py <子命令>` 或 `python app\openclaw_skills\xiaohongshu-skills\scripts\publish_pipeline.py`，不得使用其他任何实现方式。
...
```bash
# 步骤 1: 填写视频表单（不发布）
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py fill-publish-video \
  --title-file /tmp/xhs_title.txt \
  --content-file /tmp/xhs_content.txt \
...
```bash
# 图文一步到位
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py publish \
  --title-file /tmp/xhs_title.txt \
  --content-file /tmp/xhs_content.txt \
  --images "/abs/path/pic1.jpg" "/abs/path/pic2.jpg"
...
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py publish --headless \
  --title-file /tmp/xhs_title.txt \
  --content-file /tmp/xhs_content.txt \
  --images "/abs/path/pic1.jpg"

# 发布流水线（含图片下载和登录检查 + 自动降级）
python app\openclaw_skills\xiaohongshu-skills\scripts\publish_pipeline.py --headless \
  --title-file /tmp/xhs_title.txt \
  --content-file /tmp/xhs_content.txt \
  --images "https://example.com/pic1.jpg" "/abs/path/pic2.jpg"
```
...
```bash
# 指定账号
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py --account work publish \
  --title-file /tmp/xhs_title.txt \
  --content-file /tmp/xhs_content.txt \
  --images "/abs/path/pic1.jpg"

# 远程 Chrome
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py --host 10.0.0.12 --port 9222 publish \
  --title-file /tmp/xhs_title.txt \
  --content-file /tmp/xhs_content.txt \
  --images "/abs/path/pic1.jpg"
```
...
```bash
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py select-template --name "用户选择的模板名"
```
...
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py next-step \
