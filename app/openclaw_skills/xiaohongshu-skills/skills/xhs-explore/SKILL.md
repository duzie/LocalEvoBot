...
**所有搜索和浏览操作只能通过本项目的 `python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py` 完成，不得使用任何外部项目的工具：**
...
```bash
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py list-feeds
```
...
```bash
# 基础搜索
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py search-feeds --keyword "春招"
...
```bash
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py user-profile \
  --user-id USER_ID \
  --xsec-token XSEC_TOKEN
```
