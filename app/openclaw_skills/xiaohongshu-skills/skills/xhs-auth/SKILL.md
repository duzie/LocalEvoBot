...
**所有认证操作只能通过本项目的 `python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py` 完成，不得使用任何外部项目的工具：**

- **唯一执行方式**：只运行 `python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py <子命令>`，不得使用其他任何实现方式。
...
```bash
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py list-accounts
```
...
- 所有 CLI 命令位于 `app\openclaw_skills\xiaohongshu-skills\scripts\cli.py`，输出 JSON。
...
```bash
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py wait-login
```
...
```bash
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py send-code --phone <用户确认的手机号>
```
...
```bash
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py verify-code --code <用户提供的6位验证码>
```
- 自动填写验证码、点击登录。
...
```bash
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py delete-cookies
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py --account work delete-cookies  # 指定账号
```
...
```bash
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py add-account --name work --description "工作号"
# 输出: {"success": true, "name": "work", "port": 9223, "profile_dir": "..."}

python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py add-account --name personal
...
```bash
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py --account work check-login
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py --account work get-qrcode
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py --account personal check-login
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py check-login  # 不指定账号，使用默认端口 9222
```

### 管理账号

```bash
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py list-accounts                      # 列出所有账号及端口
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py set-default-account --name work    # 设置默认账号
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py remove-account --name personal     # 删除账号
```
