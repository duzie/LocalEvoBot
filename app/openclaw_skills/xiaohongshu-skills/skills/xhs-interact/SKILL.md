...
**所有互动操作只能通过本项目的 `python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py` 完成，不得使用任何外部项目的工具：**
...
```bash
# 回复指定评论（通过评论 ID）
python app\openclaw_skills\xiaohongshu-skills\scripts\cli.py reply-comment \
  --feed-id 67abc1234def567890123456 \
  --xsec-token XSEC_TOKEN \
  --content "谢谢你的分享" \
